...
export default function SelectedApplicantDetails() {
    ...
    return (
        ...
        selectedApplicantId  ?
            <div id="no_scroll" className="h-full 2xl:w-[60%] w-full text-black rounded-md flex-col justify-start items-start overflow-y-scroll p-10">
                ...
                <div className='flex w-full flex-row justify-start items-start '>
                    ...
                    <div className='flex flex-col w-full justify-start items-start'>
                        ...
                        <div className='bg-cover bg-center rounded-md font-bold text-4xl mt-5'>{selectedApplicantData.name}</div>
                        <div className='bg-cover bg-center rounded-md font-regular text-md mt-1'>{selectedApplicantData.email}</div>
                    </div>
                    ...
                    <div className='flex flex-col w-full justify-between items-end h-full mt-10'>
                        <form onSubmit={handleSubmit(SetRanking)} className='flex flex-row gap-4'>
                            <input min={1} max={10} {...register("rank")} type="number" placeholder='Rating' className=' w-24 bg-transparent border-b-2 px-2 border-tan outline-none'></input>
                            <button type="submit" className='py-2 px-4 rounded-md border-black border-[1px] text-sm hover:text-black '>Set</button>
                        </form>
...

                    {
                        selectedApplicantData.interviewInviteSent == true ?
                            <Listbox value={selectedOption} onChange={handleApplicationStatusChange}>
                                <Listbox.Button className="px-6 py-2 bg-transparent border-2 text-sm text-black border-black rounded-md hover:text-black flex flex-row justify-start items-start">
                                    {selectedOption}
                                    <IoMdArrowDropdown size={15} className="mt-1 ml-2" />
                                </Listbox.Button>
                                <Listbox.Options className={"absolute top-0 left-0 mt-10 bg-black rounded-md"}>
                                    {options.map((option) => (
                                        <Listbox.Option
                                            className="px-4 text-sm py-2 text-black hover:text-black hover:bg-blue"
                                            key={option}
                                            value={option}
                                            disabled={false}>
                                            {option}
                        ...
                        // <button onClick={() => { InterviewCandidate() }} className='bg-cover hover:scale-[1.05] hover:bg- bg-center rounded-md font-regular text-sm px-6 py-2 bg-transparent hover:text-black text-black/90 border-2 border-black w-36'>Interview</button>

                    }

                    <ButtonOutlinedWhite onClick={() => {  RejectApplicant() }} text="Reject" />

                    {/* <button onClick={() => { RejectApplicant() }} className='bg-cover hover:scale-[1.05] hover:bg- bg-center rounded-md font-regular text-sm px-6 py-2 bg-transparent hover:text-black text-black/90 border-2 border-black w-36'>Reject</button> */}


                </div>
                <form onSubmit={handleSubmit(UpdateNotes)} className='flex flex-row justify-start items-center gap-5'>
                    <textarea id="no_scroll" {...register("notes")} placeholder="Notes" className="mt-10 h-36 w-[500px] border-[1px] text-sm border-white/90 text-black/90 bg-transparent outline-0 p-3 rounded-md flex justify-center items-center">
                    </textarea>
                    <button type="submit" className='py-2 px-4 rounded-md border-black text-sm border-[1px] hover:text-black text-black'>Update</button>
                </form>
                ...
                <div className='flex flex-col justify-start items-start mt-10'>
                    {
                        selectedApplicantData.responses != null && selectedApplicantData.responses.map((response, index) => {
                            return (
                                <>
                                    <div className='text-md text-black font-regular mb-10'>
            ...
            <div className="h-full w-[60%] text-black/60 bg-blue rounded-md text-md flex flex-col justify-center items-center p-10">
