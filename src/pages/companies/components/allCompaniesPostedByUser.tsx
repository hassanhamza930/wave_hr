...
function AllCompaniesPostedByUser() {
    ...
    return (
        <div className="flex flex-col justify-start items-start my-10 gap-5">
            {
                allCompaniesPostedByUser.map((companyData) => {
                    return (
                        <button style={{ backgroundImage: `url('${companyData.companyCover}')` }} className="h-48 w-2/5 hover:scale-[1.02] rounded-md bg-cover bg-center">
                            <div className="h-full bg-cover bg-center bg-black/[95%] backdrop-hue-rotate-90 text-tan w-full rounded-md flex flex-1 flex-row justify-between items-center p-5">
                                ...
                                <div style={{ backgroundImage: `url('${companyData.companyLogo}')` }} className="h-36 w-36 rounded-md bg-cover bg-center bg-blue"></div>
