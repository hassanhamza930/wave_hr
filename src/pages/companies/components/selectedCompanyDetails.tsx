...
function SelectedCompanyDetails() {
  ...
  return (
    selectedCompany.id == null ?
      ...
      <motion.div initial={{opacity:0}} animate={{opacity:1}} id="no_scroll" className="flex justify-center items-center h-full w-full">
        <div className="text-blue text-md text-center">Select a company profile to see details</div>
      </motion.div> :

      <AnimatePresence>
        <motion.div initial={{opacity:0}} animate={{opacity:1}} id="no_scroll" className="flex-1 flex-col justify-start items-start w-full h-full rounded-md overflow-y-scroll">
